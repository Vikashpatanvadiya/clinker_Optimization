// Vercel Serverless API - Combined Backend
// This file contains all the API routes from the Express server, adapted for serverless
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (path === '/vehicles' && method === 'GET') {
      return res.json([...builtInVehicleTypes, ...customVehicleTypes]);
    }

    // Get transport modes
    if (path === '/transport-modes' && method === 'GET') {
      return res.json(transportModes);
    }

    // Create transport mode
    if (path === '/transport-modes' && method === 'POST') {
    ...
    if (path.startsWith('/transport-modes/') && method === 'PUT') {
      ...
      return res.json(transportModes[index]);
    }

    // Delete transport mode
    if (path.startsWith('/transport-modes/') && method === 'DELETE') {
      const id = parseInt(path.split('/')[2]);
      const index = transportModes.findIndex(m => m.id === id);
      if (index === -1) {
        return res.status(404).json({ error: 'Transport mode not found' });
    ...
    if (path === '/fleet/add' && method === 'POST') {
      ...
      if (existingIndex >= 0) {
        ...
        fleet.push({ ...vehicleType, count, available: true });
      }
    ...
    // Get vehicle types
    if (path === '/vehicle-types' && method === 'GET') {
    ...
    if (path === '/vehicle-types/fleet' && method === 'GET') {
      const allTypes = [...builtInVehicleTypes, ...customVehicleTypes];
      return res.json({
        vehicleTypes: allTypes,
        totalTypes: allTypes.length,
    ...
    if (path === '/vehicle-types' && method === 'POST') {
    ...
    if (path === '/vehicle-recommendations' && method === 'POST') {
      const { distance, tonnage, terrain } = req.body;
      const allVehicleTypes = [...builtInVehicleTypes, ...customVehicleTypes];
    ...
    // Smart optimization
    if (path === '/smart-optimize' && method === 'POST') {
    ...
    if (path === '/calculate-cost' && method === 'POST') {
    ...
    if (path === '/optimization-results' && method === 'GET') {
      return res.json({
        totalCost: 15420000,
